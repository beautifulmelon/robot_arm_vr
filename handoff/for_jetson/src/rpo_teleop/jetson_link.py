...
def check_robot_match(state: "State | None", urdf_name: str,
    ...
    두 가지를 본다.
      1) 팔 식별자 (URDF 파일명) — 있으면 이게 확실하다
      2) 관절 수                — 식별자가 없는 구버전 대비 (하위호환)

    ★ 왜 필요한가 — `--temp` 를 빠뜨리면 5-DOF 설정으로 3-DOF 실물에 지령을
      보낸다. 예전에는 여기서 `zip(strict=True)` 가 터져 **서버가 반쯤 뜬 채
      죽었다.** 메시지도 "argument 2 is shorter than argument 1" 이라 원인을
      알 수 없다.

      죽는 것보다 나쁜 건 **안 죽고 움직이는 것**이다. 관절 수만 보면 관절
      수가 우연히 맞는 다른 팔(예: 둘 다 3축)을 못 잡는다. 그래서 팔 식별자를
      프로토콜에 넣었다 (30_AGREED A-10).
    """
    if state is None or not state.q:
        return None
    mine = Path(urdf_name).name
    # ★ 경로는 양쪽이 다르므로 파일명만 비교한다.
    if state.robot and Path(state.robot).name != mine:
