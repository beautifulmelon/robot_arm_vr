...
def main() -> int:
    ...
    ap.add_argument("--only-joint", type=int, default=None, metavar="N",
                    help="이 관절 하나만 흔든다 (1부터). 한계 지점을 눈으로 확인할 때.\n"
                         "  예: --only-joint 1 --amp 178 --period 30")
    ...
    while running:
        ...
        if args.only_joint is not None:
            # ★ 한 관절만. 한계에 걸리는 지점을 눈으로 보려는 용도라 홈이 아니라
            #   **관절 한계 구간 전체**를 훑는다. 홈 ± amp 로 하면 홈이 가운데가
            #   아니어서 한쪽 끝을 못 본다 (joint1 홈이 -28.2° 라 그렇다).
            k = args.only_joint - 1
            q = home.copy()
            span = min(math.radians(args.amp), (hi[k] - lo[k]) / 2.0)
            mid = (lo[k] + hi[k]) / 2.0
            q[k] = mid + math.sin(phase) * span
        q = np.clip(q, lo, hi)
